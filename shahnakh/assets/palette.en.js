(() => {
 'use strict';
 const colors={navy:['Shahnakh blue','01 / DEEP BLUE','#2149a6'],rose:['Pomegranate','02 / POMEGRANATE','#d6274a'],jade:['Jade green','03 / JADE GREEN','#008f72'],lilac:['Lilac','04 / SOFT LILAC','#a457d9'],gold:['Honey gold','05 / HONEY GOLD','#dc9b23'],milk:['Milk white','06 / MILK WHITE','#eee5d4'],sky:['Sky blue','07 / SKY BLUE','#329fda'],olive:['Olive','08 / OLIVE','#7e962f'],terracotta:['Terracotta','09 / TERRACOTTA','#d65d35'],cocoa:['Cocoa','10 / COCOA','#804730'],sand:['Sand','11 / SAND','#d4ab77'],charcoal:['Charcoal','12 / CHARCOAL','#454951']};
 ...
 function showPantone(hex,button){
  ...
  document.getElementById('enquiry-color').value='PANTONE '+selectedPantone+' (approximate)';
  document.getElementById('enquiry-color-label').textContent=display;
  help.textContent=button?'Suggested Pantone reference; confirm the match with a physical fabric sample.':'Approximate suggestion from this page’s 12 references, not the full Pantone catalogue.';
 }
 ...
 function setColor(name,code,hex,button=null,instant=false){
  ...
  image.alt=`Two-thread loopback fabric, ${name}`;
  art.setAttribute('aria-label',`Two-thread fabric, ${name}; use the arrow keys to move the zoomed view`);
 ...
 document.getElementById('custom-color').addEventListener('input',event=>setColor('Custom colour','CUSTOM COLOUR',event.target.value,null,true));
 function applyPantone(){
  ...
  if(!key){hexInput.setAttribute('aria-invalid','true');help.textContent='This code is not among these 12 references. Choose a listed suggestion; your current colour is unchanged.';return;}
 ...
 document.getElementById('copy-color').addEventListener('click',async()=>{
  try{await navigator.clipboard.writeText('PANTONE '+selectedPantone);help.textContent='Pantone code '+selectedPantone+' copied; the colour match is approximate.';}
  catch{hexInput.value=selectedPantone;hexInput.focus();hexInput.select();help.textContent='Code selected. Use your device’s Copy command.';}
 });
 buttons.forEach((button,i)=>{
  button.style.setProperty('--swatch',colors[button.dataset.color][2]);
  button.title=colors[button.dataset.color][0]+' · PANTONE '+pantoneCodes[button.dataset.color]+' TCX (approximate)';
 ...
 zoom.addEventListener('input',()=>{
  ...
  document.getElementById('zoom-value').textContent=value.toLocaleString('en-US',{maximumFractionDigits:1})+'×';
  zoom.setAttribute('aria-valuetext',value.toLocaleString('en-US')+' times');
