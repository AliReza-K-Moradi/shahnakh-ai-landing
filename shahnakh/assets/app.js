(() => {
 ...
 else submitLabel.textContent='بررسی اطلاعات درخواست';
 ...
 form.addEventListener('submit',async event=>{
  ...
  const invalid=[];
  for(const key of ['fullName','brandName','fabric'])if(!data[key])invalid.push([key,'لطفاً همه موارد فرم را تکمیل کنید.']);
  if(!/^09\d{9}$/.test(phone))invalid.push(['mobile','لطفاً یک شماره موبایل معتبر ایران وارد کنید؛ مانند ۰۹۱۲۳۴۵۶۷۸۹.']);
  if(!/^\d+(\.\d{1,3})?$/.test(kg)||Number(kg)<=0||Number(kg)>100000000)invalid.push(['monthlyKg','مصرف ماهانه را به کیلوگرم و با عددی بیشتر از صفر وارد کنید.']);
  ...
  if(!endpoint){message('اطلاعات فرم معتبر است. این نسخه نمایشی است و درخواستی ارسال یا ذخیره نشده است. برای ارتباط با شاه‌نخ از شماره تماس یا لینک ادمین استفاده کنید.');return;}
  if(data.website)return;
  submit.disabled=true;submitLabel.textContent='در حال ثبت درخواست…';status.hidden=true;
  ...
  try{
   ...
   message('درخواست شما با موفقیت ثبت شد. از آشنایی با شما خوشحالیم.');form.reset();document.getElementById('enquiry-color').value='PANTONE '+document.getElementById('fabric-hex').textContent.replace(' ≈','')+' (تقریبی)';
  }catch{message('ثبت درخواست تأیید نشد. اطلاعات شما در فرم باقی مانده است. لطفاً دوباره تلاش کنید یا با ما تماس بگیرید.',true);}
  finally{clearTimeout(timeout);submit.disabled=false;submitLabel.textContent='ثبت درخواست همکاری';}
 });
})();
